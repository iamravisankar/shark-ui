...
export class ConfigurationComponent implements OnInit {
  ...
  SetLayouts(layout: string) {
    ...
    this.themeService.isDarkMode.next(layout);
  }
  ...
  SetBodyColor(theme: string) {
    ...
    this.themeService.color.next(theme);
  }
  ...
  setRtlLayout(rtl: boolean) {
    if (rtl) {
      ...
      this.rtlLayout = true;
      this.themeService.directionChange.next(RTL);
    } else {
      this.renderer.removeClass(document.body, 'able-pro-rtl');
      this.rtlLayout = false;
      this.themeService.directionChange.next(LTR);
    }
  }
  ...
  setMenuOrientation(layout: string) {
    ...
    this.themeService.layout.next(layout);
  }
}
